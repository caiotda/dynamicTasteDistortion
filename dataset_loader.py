...
from simulationConstants import MOVIELENS_PATH, USER_COL, ITEM_COL, GENRES_COL
size_to_file_name = {
    "s": "1m",
    "m": "10m",
    "l": "20m"
...
def download(size):
    ...
    if os.path.exists(file_name):
        os.remove(file_name)
    file_name_length = file_name.rfind(".")
    file_name_cleaned = "ml-10M100K" if size == "m" else file_name[:file_name_length]
    return destination_dir + file_name_cleaned

def read_ml_raw(size):
    files_dir = download(size)
...
def filter_inactive_users(df, threshold=REVIEWS_PER_USER_THRESHOLD):
    users_to_keep = df.groupby(USER_COL).agg({ITEM_COL: "count"}).reset_index().rename(columns={ITEM_COL: "n_reviews"}).query(f"n_reviews >= {threshold}")[USER_COL]
...
def process_ml_df(df):
    ...
    user_id_map = {old_id: new_id for new_id, old_id in enumerate(sorted(unique_user_ids))}
    item_id_map = {old_id: new_id for new_id, old_id in enumerate(sorted(unique_item_ids))}
    ...
    filtered_df["binarized_rating"] = filtered_df["rating"].apply(lambda rating: int(rating >= 4))
    return filtered_df
...
def main():
    ...
    parser.add_argument('--size', choices=['s', 'm', 'l'], required=True, help='Dataset size: s (1m), m (10m), l (20m)')
    parser.add_argument('--data', choices=['ml'], required=True, help='Dataset type: ml (MovieLens)')
